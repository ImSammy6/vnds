...
void ScrollPane::SetScroll(int s, bool isChoice) {
	if (isChoice)
		s = MAX(0, MIN(totalItemL-visibleItemL, s));
	else
		s = MAX(0, MIN(totalItemL-numLines, s));
	...
	if (scroll != oldScroll) {
		...
		if (delta > 0 && delta < visibleItemL+5) {
