...
void ScriptEngine::ParseCommand(Command* cmd, char* data) {
    ...
    if (name[0] == '#') {
		...
		if (readInt(&fadeStart)) {
    		cmd->bgload.fadeStart = fadeStart;
    	} else {
    		cmd->bgload.fadeStart = -1;
    	}
		if (readInt(&fadeEnd)) {
    		cmd->bgload.fadeEnd = fadeEnd;
    	} else {
    		cmd->bgload.fadeEnd = -1;
    	}
		if (readInt(&ignorePrev)) {
    		cmd->bgload.ignorePrev = ignorePrev;
    	} else {
    		cmd->bgload.ignorePrev = -1;
    	}
		...
		int fadeTime;
		if (readInt(&fadeTime)){
			cmd->setimg.fadeTime = fadeTime;
		} else {
			cmd->setimg.fadeTime = 16;
		}
