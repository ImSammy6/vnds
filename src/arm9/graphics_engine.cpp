...
int GE_FADE_FRAMES = 16;
bool fadeIgnorePrev = false;
		...
		int frames=GE_FADE_FRAMES)
{
	...
	if (!fadeIgnorePrev)
		...
		REG_BLDCNT_SUB = BLEND_ALPHA | BLEND_SRC_SPRITE;
...
void GraphicsEngine::SetBackground(const char* filename, s16 fadeTime, bool ignorePrev) {
    backgroundChanged |= (strcmp(backgroundPath, filename) != 0);
    strcpy(backgroundPath, filename);
    backgroundFadeTime = fadeTime;
	fadeIgnorePrev = ignorePrev;
...
void GraphicsEngine::SetForeground(const char* filename, s16 x, s16 y, int fadeTime) {
    if (spritesL >= GE_MAX_SPRITES) {
    	vnLog(EL_warning, COM_GRAPHICS, "Maximum number of sprites reached");
        return;
    }

	GE_FADE_FRAMES = fadeTime;
