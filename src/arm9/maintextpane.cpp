...
void MainTextPane::Update(u32& down, u32& held, touchPosition touch) {
	...
	if (textSpeed > 0) {
		...
		if (activeVisibleChars < strL || activeVisibleChars == 9999) {
			for (int n = currentChunkStart; n < GetNumberOfItems(); n++) {
				int offset = activeVisibleChars+lineStarts[currentChunkStart];
				if (instant || (offset-lineStarts[n] >= 0 && offset-lineStarts[n+1] <= 0)) {
					if (offset-lineStarts[n+1] > -lineSkips[n+1]){ // fixes pause between lines, but ends lines early if they use default color
						activeVisibleChars += lineStarts[n+1]-offset;
					}
					SetItemDirty(n);
				}
			}
...
bool MainTextPane::AppendText(const char* text, u16 color, bool stripSpaces) {
	...
	bool instantText = (text[0] == '\\' && strncmp(text, "\\i", 2) == 0) || (text[1] == '\\' && strncmp(text+1, "\\i", 2) == 0);
	// ^ makes instant text show all lines while skipping
	if (instantText)
		activeVisibleChars = 9999;
	else
