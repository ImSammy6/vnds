...
void ScriptInterpreter::cmd_setimg(Command* cmd, bool quickread) {
	...
	vnds->graphicsEngine->SetForeground(path, cmd->setimg.x, cmd->setimg.y, cmd->setimg.fadeTime);
}
void ScriptInterpreter::cmd_bgload(Command* cmd, bool quickread) {
	...
	vnds->graphicsEngine->SetBackground(path, cmd->bgload.fadeTime, cmd->bgload.ignorePrev == 1);
}
