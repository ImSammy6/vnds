...
SoundEngine::SoundEngine(Archive* soundArchive) {
	...
	if (isDSiMode())
		SOUND_BUFFER_LENGTH = (96 * 1024 * 6);
	else
		SOUND_BUFFER_LENGTH = (96 * 1024);

	u8 soundBuffer[SOUND_BUFFER_LENGTH] = {0};
...
void SoundEngine::PlaySound(const char* path, int times) {
	...
	if (strcmp(path, "sound/~") == 0) {
	    ...
	    if (periodChar && (tolower(periodChar[1])=='a' || tolower(periodChar[1])=='m')) {
			...
			if (aacPlayer->PlaySound(soundArchive, soundPath, preferences->GetSoundVolume(), times < 0)) {
