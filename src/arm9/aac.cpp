            ...
            if (bufferReadPos >= BUFFER_SIZE) {
                ...
                if (bufferStopPos >= 0) {
                    if (oldReadPos < bufferStopPos || bufferReadPos >= bufferStopPos) {
                        if (loopCurrentSound) aacPlayer->PlaySound(currentArchive, currentFilename, currentVolume, true);
                        else playing = false;
                        //iprintf("finished");
                    }
                }
...
bool AACPlayer::PlaySound(Archive* archive, char* filename, u8 volume, bool loopSound) {
    StopSound();
